...
- batch.max.rows: 한번에 가져오는 batch의 최대 row 수
    ...
    - bulk모드의 "batch.max.rows" 옵션은 다른모드일 때와 작동방식이 약간 다르다.
        ...
        - timestamp, incrementing: (1회 polling시 DB에서 가져오는 분량) = (batch.max.rows 크기)
    ...
    - bulk모드로 배치성 쿼리작업시, 쿼리주기가 쿼리소요시간만큼 계속 지연되는 것에 주의
        - Kafka는 배치작업에 맞지 않다.
        - 데이터가 항상 거의 일정한 코드성테이블들은 가능한 디비지움(CDC)or CT(Change Tracking) 등을 쓰는 것이 좋다.
        - 또는 깔끔하지는 않지만 REST API로 connector를 on-off하는 것이 직관적이고 구현하기는 쉽다.
