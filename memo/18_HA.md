...
- Broker Config(server.properties)에서

```yaml
# https://kafka.apache.org/42/configuration/

################################################################################
# [Broker Config에서 Topic 기본값 변경시]
# 1. 파티션 수/RF 관련: 신규생성 토픽에만 적용 (기존 토픽에 소급적용 안됨)
# 2. 나머지 전부: 기존 토픽에 소급 적용 (단, 개별 topic-level 설정이 있으면 topic-level 우선)
################################################################################

################################################################################
# [HA설정]
# 설정가능 범위:  1 <= minISR <= RF <= 브로커 수
# 권장값: [RF=3, minISR=2]

# 보통 topic-level에서 관리되지만, 그 기본값을 server.properties에서 지정가능
# 기존 topic의 RF변경은 kafka-reassign-partitions.sh 스크립트로 수동작업 필요
# 중요한 topic일 수록 RF가 크면 좋음(e.g. __consumer_offset, connect-offset 등)
################################################################################

# 신규생성 토픽의 파티션 수 (브로커 수의 정수배 권장) (default: 1)
num.partitions: 3

# __consumer_offset의 RF수 (default: 3)
offsets.topic.replication.factor: 3

# 신규생성 토픽의 RF수 (default: 1)
default.replication.factor: 3

# 모든 토픽의 minISR기본값 (default: 1)
